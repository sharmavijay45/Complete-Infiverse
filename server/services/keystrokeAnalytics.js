...
const keystrokeAnalyticsSchema = new mongoose.Schema({
  ...
  productivity_metrics: {
    ...
    productivity_reasoning: { type: String, default: '' }
  },
  pattern_analysis: {
...
class KeystrokeAnalyticsService {
  ...
  analyzeProductivityMetrics(buffer, analysis) {
    ...
    if (totalTime > 0) {
      ...
      // Add reasoning
      let reasoning = `Productivity score is based on: `;
      reasoning += `Active time (${Math.round(activeRatio * 100)}%), `;
      reasoning += `typing speed (${analysis.keystroke_data.typing_speed_wpm} WPM), `;
      reasoning += `and accuracy (${analysis.keystroke_data.typing_accuracy}%). `;

      if (activeRatio < 0.5) {
        reasoning += "Score dropped due to high idle time. ";
      }
      if (speedFactor < 0.7) {
        reasoning += "Typing speed is below average. ";
      }
      if (accuracyFactor < 0.9) {
        reasoning += "Typing accuracy is low. ";
      }

      analysis.productivity_metrics.productivity_reasoning = reasoning;
    }
