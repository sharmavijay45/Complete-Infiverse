...
router.post('/start-day/:userId', auth, async (req, res) => {
  try {
    ...
    // Update today's aim with work location if it exists, or create one if needed
    ...
    if (todayAim) {
      ...
      // Create a default aim with work location if none exists
      ...
      await todayAim.save();
      console.log(`🎯 Created default aim with work location ${workLocationType} for user ${userId}`);
    }
